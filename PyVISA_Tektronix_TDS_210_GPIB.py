...
class TDS210():
    ...
    TDS_Dic={'Functions':['**Select Command To Execute**',
        '************ UTILITIES ************','oscope_self_cal','oscope_auto_set','oscope_factory_set',
        'oscope_reset','oscope_clear','oscope_frontpanel_lock, NONE','oscope_save_setup, 1','oscope_recall_setup, 1',
        '************* DISPLAY *************','set_disp_style, VECTORS','set_disp_persistence, OFF','set_disp_format,YT',
        'set_disp_contrast, 75','get_display_style','get_display_persistence','get_display_format','get_display_contrast',
        '************ VERTICAL ************','set_vert_select, CH1, ON','set_vert_bandwidth, OFF, CH1',
        ...
        '*********** HORIZONTAL ***********','set_horiz_main_scale, 5.0E-4','set_horiz_main_position, 0.0',
        'set_horiz_view, MAIN','set_horiz_delay_scale, 250E-6','set_horiz_delay_position, 0.0','get_horiz_all',
        'get_horiz_main','get_horiz_main_scale','get_horiz_main_position','get_horiz_view','get_horiz_delay_scale',
        'get_horiz_delay_position','************* TRIGGER *************','set_trig_force','set_trig_50%',
        'set_trig_mode, AUTO','set_trig_type, EDGE','set_trig_level, 1.5','set_trig_edge_coupling, DC',
        'set_trig_edge_slope, RISE','set_trig_edge_source, CH1','set_trig_holdoff_value, 5.0e-7',
        'set_trig_video_polarity, NORMAL','set_trig_video_source, CH1','set_trig_video_sync, LINE','get_trig',
        'get_trig_main','get_trig_mode','get_trig_type','get_trig_level','get_trig_edge_coupling','get_trig_edge_slope',
        'get_trig_edge_source','get_trig_holdoff','get_trig_video_polarity','get_trig_video_source','get_trig_video_sync',
        ...
        '************* DATA/CURVE *************','set_data_init','set_data_destination, REFA','set_data_encoding, ASCII',
        'set_data_source, CH2','set_data_start, 1','set_data_stop, 2500','set_data_width, 1','get_data_info',
        'get_curve_data','get_data_destination','get_data_encoding','get_data_source','get_data_start','get_data_stop',
        'get_data_width','************* CURSOR *************','set_cursor_type, HBARS','set_cursor_source, CH1',
        'set_hbars_pos1, 0.0','set_hbars_pos2, 5.5','set_vbars_pos1, -2.0E-3','set_vbars_pos2, 2.0E-3',
        'set_vbars_units, SECONDS','get_cursor','get_cursor_type','get_cursor_source','get_hbars_pos1','get_hbars_pos2',
        'get_hbars_units','get_cursor_hbars','get_hbars_delta','get_vbars_pos1','get_vbars_pos2','get_vbars_units',
        'get_cursors_vbars','get_vbars_delta','get_ymult','get_yoff','get_yzero'],
        #/////////////////////////////////////////////ARGUMENTS///////////////////////////////////////////////////
        ...
        'Value: (None) Returns Immed Value','Units: (None) Returns Immed Units','**** DATA AND CURVE ****',
        ...
        'Curve: (None) Retrieve Data Specified By Data Source, Data Start, Data Stop, Data Width',
        ...
        'Width: (None) Retrieves The Number Of Bytes Per Waveform Data Point To Be Transfered','**** CURSOR ****',
        'Type: (HBARS, VBARS, OFF) Sets The Curson Type','Source: (CH1, CH2, MATH, REFA, REFB) Sets Type For Vertical And Horizontal Cursors',
        ...
        'Delta (None) Returns Difference Between The 2 Vertical Bars In Vertical Units','YMULT','YOFF','YZERO']}
    def __init__(self, function): # function Contains Function Name And Arguments. See TDS_Dic['Functions']
        super().__init__()
        ...
        for i, items in enumerate(TDS210.TDS_Dic['Functions']):# Find Index Based On Function Text Instead Of Using cbo Index.
    ...
    def __str__(self):
        def gpib_initialize():#********** Controller  Initialization **********
            try:
                ...
                self.res=self.rm.list_resources() # Time Consuming
                ctrl=self.rm.open_resource('GPIB::INTFC')
                interface_num=ctrl.interface_number
                interface_name["text"]=ctrl.resource_manufacturer_name
                interface_address["text"]='Controller'+str(interface_num)+ ' @ '+str(ctrl.resource_name)[ctrl.resource_name.find("'")+1:ctrl.resource_name.find(':')]
                ...
                interface_stat["text"]=self.rtn_val
                msg1=interface_name["text"]+' Controller Initialization Complete!\n'
                if len(self.res)!=0:msg2='Instruments Detected @ '+str(self.rm.list_resources())
                else:msg2='No Instruments Detected Connected To GPIB Bus!'
                messagebox.showinfo(interface_name["text"]+' GPIB Bus Initialization', msg1+msg2)
                Gpib_Initialized=True
                return(self.rtn_val)
            except Exception as e:
                self.rtn_val='Initialization Failed'
                interface_stat["text"]='Initialization Failed'
                ...
                return 'break'
        def oscope_initialize():#********** Instrument Initialization **********        
            try:
                global Oscope
                global Oscope_Initialized
                if len(self.res)>=1:
                    oscope_port=str(self.ctrl_port+'::'+self.oscope_address+'::INSTR')
                    if oscope_port in self.res:
                        ...
                        if idn!='0' and idn!='':
                            ...
                            Oscope.write("DISPLAY:CONTRAST 70")
                            oscope_stat["text"]='Initialization Passed'
                            name=idn.split(',')
                            oscope_name["text"]=name[0]+ ' '+name[1]
                            oscope_address["text"]='Oscilloscope @ '+str('GPIB'+ self.oscope_address)
                            msg1=oscope_name["text"]+' Initialization Complete!\n'
                            msg2='Oscilloscope Detected @ '+str(oscope_port)
                            msg3='\nOscilloscope IDN: '+str(idn)
                            messagebox.showinfo(oscope_name["text"]+' Oscilloscope Initialization',msg1+msg2+msg3)
                            Oscope_Initialized=True
                            self.rtn_val='Pass'
                            return(self.rtn_val)
                    else:
                        self.rtn_val='Failed'
                        oscope_stat["text"]=self.rtn_val
                        ...
                        Oscope_Initialized=False
                        self.rtn_val='Failed'
                        return(self.rtn_val)
            except Exception as e:
                self.rtn_val='Initialization Failed'
                oscope_stat["text"]='Initialization Failed'
                msg1='Exception occurred while code execution:\n'
        ...
        def oscope_self_cal():# Returns "PASS", "FAIL"        
            try:
                ...
                query.focus()
                query.delete('1.0','end')
                query.insert('end','Self Calibration FAILED')
                msg1='Exception occurred while code execution:\n'
        ...
        def oscope_auto_set():    
            try:
                ...
                query.focus()
                query.delete('1.0','end')
                query.insert('end','Autoset OK')
                return(self.rtn_val)
            except Exception as e:
                query.focus()
                query.delete('1.0','end')
                query.insert('end','Autoset Failed')
                msg1='Exception occurred while code execution:\n'
        ...
        def gpib_send():
            ...
            TDS_CMDS={'Commands':['*******','****UTIL****','CAL:INTERNAL','AUTOSET EXECUTE','FACTORY','*RST',
                        ...
                        'MEASUREMENT:IMMED:VALUE?','MEASUREMENT:IMMED:UNITS?','****DATA****','DATA INIT',
                        ...
                        'DATA?','CURVE?','DATA:DESTINATION?','DATA:ENCDG?','DATA:SOURCE?','DATA:START?',
                        'DATA:STOP?','DATA:WIDTH?', '**** CURSOR ****',str('CURSOR:FUNCTION '+self.args[1]),
                        ...
                        'CURSOR?','CURSOR:FUNCTION?','CURSOR:SELECT:SOURCE?','CURSOR:HBARS:POSITION1?',
                        'CURSOR:HBARS:POSITION2?','CURSOR:HBARS:UNITS?','CURSOR:HBARS?','CURSOR:HBARS:DELTA?',
                        'CURSOR:VBARS:POSITION1?','CURSOR:VBARS:POSITION2?','CURSOR:VBARS:UNITS?','CURSORS:VBARS?',
                        'CURSOR:VBARS:DELTA?','WFMPRE:YMULT?','WFMPRE:YOFF?','WFMPRE:YZERO?']}
            try:
                if not '?' in TDS_CMDS['Commands'][self.index]:# WRITES
                    Oscope.write(TDS_CMDS['Commands'][self.index])
                    self.rtn_val=self.funct+ ' Passed'
                    query.delete('1.0','end')
                    query.focus()
                    query.insert('end',self.rtn_val)
                else: #QUERIES  
                    if TDS_CMDS['Commands'][self.index]=='CURVE?':# Convert Digitized Value To Y Values (Volts)
                        Oscope.write('DATA:ENCDG ASCII') # Make Sure Formatting Is ASCII                   
                        self.rtn_val=Oscope.query(TDS_CMDS['Commands'][self.index])
                        val=Oscope.query('WFMPRE:YMULT?').split(" ")
                        ymult=float(val[1])
                        val=Oscope.query('WFMPRE:YOFF?').split(" ")
                        yoffset=float(val[1])
                        val=Oscope.query('WFMPRE:YZERO?').split(" ")
                        yzero=float(val[1])
                        val=Oscope.query('WFMPRE:NR_PT?').split(" ")
                        num_pts=int(val[1])
                        val=Oscope.query('WFMPRE:XINCR?').split(" ")
                        xinc=float(val[1])
                        curve_x=[]
                        curve_y=[]
                        xnow=0.0
                        for p in range(0,num_pts):
                            ...
                            xnow+=xinc
                        digitized=self.rtn_val.replace(':CURVE ','')# Remove ':CURVE ' From String
                        ...
                        Curve_XY = [i for i in zip(curve_x, curve_y)]
                        Waveform_Data.focus()
                        Waveform_Data.delete('1.0','end')
                        Waveform_Data.insert('end','X Values In Seconds And Y Values In Volts (X, Y)\n')
                        Waveform_Data.insert('end',str(Curve_XY))
                        self.rtn_val=str(Curve_XY)
                    else:
                        self.rtn_val=Oscope.query(TDS_CMDS['Commands'][self.index])
                        sent_lbl.focus()
                        sent_lbl.config(text=TDS_CMDS['Commands'][self.index])
                ...
                msg1='Exception occurred while code execution:\n'
                msg2='Bus Command = '+TDS_CMDS['Commands'][self.index]
...
def execute_cmd(event):
    ...
    response=str(TDS210(cmd))# Combobox Selection
    return 'break'
def set_choices(event):# ComboBox Selection Made, Update arguments_lbl
    Selected_Index.set(select.current())
    arguments_lbl.focus()
    arguments_lbl.config(text='Argument Choices: '+str(TDS210.TDS_Dic['Arguments'][Selected_Index.get()]))
...
def about():
    messagebox.showinfo('About PyVISA Tektronix TDS 210', 'Creator: Ross Waters\nEmail: RossWatersjr@gmail.com'\
        '\nNI VISA For Windows 11 64 Bit, (2022 Q3) Or 2023 Q2'\
        '\nPyVISA 1.13.0 Or Latest Version'\
        '\nLanguage: Python version 3.11.2 64-bit'\
        '\nRevision \ Date: 1.0 \ 06/17/2023'\
        '\nTested With (NI GPIB-USB-HS+) Controller')
if __name__ == "__main__":
    ...
    title=('PyVISA Tektronix TDS 210/220 Oscilloscope')
    options='Right Click Window For About'
    root.title(title + options.rjust(45+len(options)))
    ...
    select['values']=TDS210.TDS_Dic['Functions']
    select['state']='normal'
    ...
    Waveform_Data.delete('1.0','end')
    Waveform_Data.insert('end','Curve Data')
    Gpib_Initialized=False
    Oscope_Initialized=False
    response=str(TDS210('oscope_initialize'))# Initialize Controller And Instrument
    root.mainloop()    
